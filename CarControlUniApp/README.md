...
基于 Vue 3 与经典 uni-app 的 Q60S 车辆控制界面。App 会优先恢复系统返回的首个已连接 `FFF0` 设备；没有可恢复设备时，主动扫描名称包含 `RM0` 或 `RM3` 的设备。两条路径都只在发现 `FFF0` 服务下的可写特征后启用控制按钮。
...
- 已连接 FFF0 设备恢复、RM0/RM3 扫描过滤、连接状态、重新扫描和调试日志
...
5. 首次进入时允许“附近设备/蓝牙”和定位权限；部分新系统仍会要求位置权限，拒绝后系统设置能看到车辆设备，但 App 无法获得 BLE 扫描结果。
...
- 小程序蓝牙使用相同策略：优先恢复已连接的 FFF0 设备，否则扫描 RM0/RM3；使用时必须由用户在小程序内发起连接。
...
- [ ] 已连接路径恢复系统返回的首个 FFF0 设备，不额外按名称过滤
- [ ] 没有已连接 FFF0 设备时，主动扫描接受名称包含 RM0 或 RM3 的设备
